...
class GoogleSearchExecutor : ActionExecutor {
    override fun execute(command: ParsedCommand, context: Context): ActionResult {
        ...
        return try {
            ...
            val successMessage = if (query.isBlank()) "Opening Google" else "Searching Google for $query"
            ActionResult(true, successMessage)
        } catch (e: Exception) {
            ActionResult(false, "Failed to open browser: ${e.message}")
        }
    }
}
